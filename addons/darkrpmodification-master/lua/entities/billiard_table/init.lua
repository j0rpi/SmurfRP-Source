...
function ENT:Use(ply, caller)
	...
	if ply:KeyDown(IN_WALK) then -- Open config for admins
	...
	if self.PlayerData && self.PlayerData[1] then
		...
		if b_error then
			ply:PrintMessage(HUD_PRINTTALK, "Error! "..b_error)
			return
		...
		ply:PrintMessage(HUD_PRINTTALK, "[Billiards] A request has been sent! Please wait for an answer!")
	...
	if(self.ThreeCFouls) then
		...
		if(self.Foul) then
			...
			if(self.PlayerData[id].Fouls == 2) then -- Warn him about the 2 fouls
				ply:PrintMessage(HUD_PRINTTALK, "[Billiards] WARNING! You already have fouled twice! Fouling again will result in loss of game!")
