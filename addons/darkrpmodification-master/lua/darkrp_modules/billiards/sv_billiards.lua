...
function CBilliardMingeProtection(ply, ent)
	if(table.Count(svPBilliardTables) >= 1) then
		...
		for k,v in pairs(svPBilliardTables) do
			if(ent:GetClass() == "billiard_table" && ent.ID == v) then 
				if !ply:IsAdmin() then
					return false
				end
			elseif(ent.BilliardTableID && ent.BilliardTableID == v) then 
				if !ply:IsAdmin() then
					return false
				end 
			end
...
hook.Add("OnBilliardGameEnd", "EndingGameBilliard", function(ptable, winner, loser)
	winner:SendLua("chat.AddText(Color(0,255,0,255), '[BILLIARDS] ', Color(255,255,255,255), 'Winner! Congratulations!')")
	if(loser) then
		loser:SendLua("chat.AddText(Color(0,255,0,255), '[BILLIARDS] ', Color(255,255,255,255), 'You Lost, better luck next time!')")
	end
end)
------------------------------------------------------------
...
hook.Add("Move", "CBilliardMove", function(ply, movedata)
	...
	if(ptable) then
		...
		if(b_bool && !ply.WarnAreaSent) then
			...
			ply:SendLua("chat.AddText(Color(0,255,0,255), '[BILLIARDS] ', Color(255,255,255,255), 'You have five seconds to get back to play area or you will lose the game!')")
		...
		elseif(!b_bool && ply.WarnAreaSent) then
			...
			ply:SendLua("chat.AddText(Color(0,255,0,255), '[BILLIARDS] ', Color(255,255,255,255), 'Welcome back to the Play Area!')")
		end
...
concommand.Add("billiard_config", function(ply, cmd, args)
	if !IsValid(ply:GetEyeTraceNoCursor().Entity) then return end
	local ptable = ply:GetEyeTraceNoCursor().Entity
	if !ply:IsAdmin() then return end
	
	if not ptable:IsValid() then return end 
	if ptable:GetClass() != "billiard_table" then return end
...
concommand.Add("billiard_acc_ref", function(ply, cmd, args)
	...
	if(tobool(args[1])) then
		...
		ply:SendLua("chat.AddText(Color(0,255,0,255), '[BILLIARDS] ', Color(255,255,255,255), 'You are now playing Billiards!')")
		...
		reqply:SendLua("chat.AddText(Color(0,255,0,255), '[BILLIARDS] ', Color(255,255,255,255), 'Your Game Request was refused!')")
	end
end)
